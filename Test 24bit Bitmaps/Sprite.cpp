...
int CSPRITE::LoadSpriteImage24(char * filename)
{
	...
	for(int frame=0; frame< m_iNumberofFrames; frame++)
	{
		...
		if(!Lock(m_lpddsImage[frame]))
			...
			{
			    // copy next line of data to destination
				memcpy(dest_ptr, source_ptr,m_iWidth*4);//4 for four bytes
...
int CSPRITE::DrawSprite(LPDIRECTDRAWSURFACE7 lpddsSurface, int nX, int nY)
{
	...
	if( (m_iState != SPRITE_STATE_DEAD) && ( m_iAttr == SPRITE_ATTR_LOADED) )
	{
		...
		destRect.top = nY;
		destRect.left = nX;
		destRect.bottom = nY + m_iHeight;
		destRect.right = nX + m_iWidth;
		...
		if(ddReturnVal != DD_OK)
			...
			{	this->RestoreSurface(); lpddsPrimary->Restore(); OutputDebugString("Surfacelost DrawSprite(x,y)\n"); return(1);	}
			else
