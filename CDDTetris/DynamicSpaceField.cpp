...
int DynamicStarField(RECT *starField, int nNumofStars, CVECTOR velVec, int nFrame)
{
	...
	DrawStars(stars, nNumofStars, GetGameApp()->getSecondarySurf());
...
inline int InitStars(CUtilities::PARTICLE *stars, RECT *starField, RECT &tempField, int nNumofStars)
{
	...
	if(starField == NULL) //if the user dosn't specify a size use the whole screen
	{
		...
		tempField.bottom = GetGameApp()->getSettings().GetSettings()->nBackBufferHeight; tempField.right = GetGameApp()->getSettings().GetSettings()->nBackBufferWidth;
	}
...
inline int DrawStars(const CUtilities::PARTICLE *stars, int nTotalAmountofStars, LPDIRECTDRAWSURFACE7 lpddsSurface)
{
	...
	for(int i=0; i<nTotalAmountofStars; i++)
		GetGameApp()->getUtlities().DrawPixel32(ddsd,(int)stars[i].x,(int)stars[i].y, stars[i].r,stars[i].g,stars[i].b);
...
inline int MoveStars(CUtilities::PARTICLE* stars, int nNumofStars, RECT field, CVECTOR velVec)
{
	for(int i=0; i<nNumofStars; i++)
	{
		...
		{
			stars[i].x += velVec.x*L3_SPEED_REDUC_PERCENT*GetGameApp()->getTimer().GetTimeElapsed();
			stars[i].y += velVec.y*L3_SPEED_REDUC_PERCENT*GetGameApp()->getTimer().GetTimeElapsed();
		}
		else
			if(stars[i].fadefactor == L2_FADE_AMOUNT)
			{
				stars[i].x += velVec.x*L2_SPEED_REDUC_PERCENT*GetGameApp()->getTimer().GetTimeElapsed();
				stars[i].y += velVec.y*L2_SPEED_REDUC_PERCENT*GetGameApp()->getTimer().GetTimeElapsed();
			}
			else
			{
				stars[i].x += velVec.x*GetGameApp()->getTimer().GetTimeElapsed();
				stars[i].y += velVec.y*GetGameApp()->getTimer().GetTimeElapsed();
			}
...
inline void DebugInfo(const CUtilities::PARTICLE *stars, int nFrame, int nNumofStars, RECT field, LPDIRECTDRAWSURFACE7 lpddsSurface)
{
	...
	GetGameApp()->getUtlities().DrawStringGDI(lpddsSurface,0,30,cpErrorBuf);
	sprintf(cpErrorBuf,"Init x[0]= %d", (int)stars[0].x);
	GetGameApp()->getUtlities().DrawStringGDI(lpddsSurface,0,45,cpErrorBuf);
	sprintf(cpErrorBuf,"Init y[0]= %d", (int)stars[0].y);
	GetGameApp()->getUtlities().DrawStringGDI(lpddsSurface,0,60,cpErrorBuf);
	sprintf(cpErrorBuf,"Init velX0= %g", stars[0].vx0);
	GetGameApp()->getUtlities().DrawStringGDI(lpddsSurface,0,75,cpErrorBuf);
	sprintf(cpErrorBuf,"Init velY0= %g", stars[0].vy0);
	GetGameApp()->getUtlities().DrawStringGDI(lpddsSurface,0,90,cpErrorBuf);
	sprintf(cpErrorBuf,"# of L1 stars= %g", nNumofStars*L1_PERCENT_OF_STARS);
	GetGameApp()->getUtlities().DrawStringGDI(lpddsSurface,0,105,cpErrorBuf);
	sprintf(cpErrorBuf,"# of L2 stars= %g", nNumofStars*L2_PERCENT_OF_STARS);
	GetGameApp()->getUtlities().DrawStringGDI(lpddsSurface,0,120,cpErrorBuf);
	sprintf(cpErrorBuf,"# of L3 stars= %g", nNumofStars*L3_PERCENT_OF_STARS);
	GetGameApp()->getUtlities().DrawStringGDI(lpddsSurface,0,135,cpErrorBuf);
	sprintf(cpErrorBuf,"Rect= %d,%d,%d,%d", field.left,field.top,field.right,field.bottom);
	GetGameApp()->getUtlities().DrawStringGDI(lpddsSurface,0,150,cpErrorBuf);
}