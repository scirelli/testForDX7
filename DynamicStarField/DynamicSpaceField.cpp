...
//---------------------------------------------------------------
int InitStars(PARTICLE *stars, RECT *starField, RECT &tempField, int nNumofStars)
{
	...
	if(starField == NULL) //if the user dosn't specify a size use the whole screen
	{
		...
		tempField.bottom = WINDOW_HEIGHT; tempField.right = WINDOW_WIDTH;
	}
...
//---------------------------------------------------------------
int DrawStars(const PARTICLE *stars, int nTotalAmountofStars)
{
...
//---------------------------------------------------------------
int MoveStars(PARTICLE* stars, int nNumofStars, RECT field, CVECTOR velVec)
{
	for(int i=0; i<nNumofStars; i++)
	{
		if(stars[i].fadefactor == L3_FADE_AMOUNT)
		{
			stars[i].x += velVec.x*L3_SPEED_REDUC_PERCENT;
			stars[i].y += velVec.y*L3_SPEED_REDUC_PERCENT;
		}
		else
			if(stars[i].fadefactor == L2_FADE_AMOUNT)
			{
				stars[i].x += velVec.x*L2_SPEED_REDUC_PERCENT;
				stars[i].y += velVec.y*L2_SPEED_REDUC_PERCENT;
			}
			else
			{
				stars[i].x += velVec.x;
...
//---------------------------------------------------------------
void DebugInfo(const PARTICLE *stars, int nFrame, int nNumofStars, RECT field)
{
