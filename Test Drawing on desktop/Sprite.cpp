...
void CSPRITE::SetAnimations(int * anim, int SeqMax, int attribute, int AnimIndex)
{
	if(AnimIndex > MAX_SPRITE_ANIMATIONS)
		AnimIndex = MAX_SPRITE_ANIMATIONS;

	m_ipAnimations[AnimIndex]= anim;
	m_ipAnimSeqMax[AnimIndex] = SeqMax;
	m_ipAnimAttr[AnimIndex] = attribute;
}
...
//-----------------------------------------------------------------------------   
int Flip(LPDIRECTDRAWSURFACE7 lpddsSurface)
{// this function flips a surface onto the primary surface
	...
	temp.bottom -=4;
	//Blt the frame
	ddReturnVal = lpddsPrimary->Blt(&temp/*dest rect*/,lpddsSecondary, //pointer to source surface
									  NULL, //pointer to the source rectangle
									  DDBLT_WAIT | DDBLT_KEYSRC, NULL/*ddbltfx struct*/);//NULL means entire surface
	if (DDFailedCheck(ddReturnVal, "Flip(lpddsSurface), Blt failed", cpErrorBuf ))
	{	MessageBox(main_window_handle, cpErrorBuf, "DrawSprite()", MB_ICONEXCLAMATION);   return(0); }
